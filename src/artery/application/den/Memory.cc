...
vanetza::Clock::time_point Reception::expiry() const
{
    ...
    const DenmManagementContainer_t& denmManagement = (*message)->denm.management;
...
void Memory::received(const DenmObject& denm)
{
    ...
    if (found == idx_action_id.end()) {
        ...
        const DenmManagementContainer_t& stored = (*found->message)->denm.management;
        const DenmManagementContainer_t& received = denm.asn1()->denm.management;
