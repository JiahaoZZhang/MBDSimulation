...
vanetza::asn1::Denm UseCase::createMessageSkeleton()
{
    ...
    message->header.messageId = MessageId_denm; //ItsPduHeader__messageID_denm;
    ...
    message->denm.management.eventPosition.longitude = round(mVdp->longitude(), microdegree) * 10; // Longitude_oneMicrodegreeEast;
    message->denm.management.eventPosition.latitude = round(mVdp->latitude(), microdegree) * 10; // Latitude_oneMicrodegreeNorth;
    ...
    message->denm.location->eventSpeed->speedValue = std::abs(round(mVdp->speed(), centimeter_per_second)) * 1; // SpeedValue_oneCentimeterPerSec;
    message->denm.location->eventSpeed->speedConfidence = 1 * 3; // SpeedConfidence_equalOrWithinOneCentimeterPerSec
    ...
    message->denm.location->eventPositionHeading->headingConfidence = 10; // HeadingConfidence_equalOrWithinOneDegree
