...
vanetza::asn1::Cam RsuCaService::createMessage() const
{
    ...
    basic.stationType = TrafficParticipantType_roadSideUnit; // StationType_roadSideUnit;
    basic.referencePosition.altitude.altitudeValue = AltitudeValue_unavailable;
    basic.referencePosition.altitude.altitudeConfidence = AltitudeConfidence_unavailable;
    const double longitude = mGeoPosition->longitude / vanetza::units::degree;
    basic.referencePosition.longitude = std::round(longitude * 1e6 * 10); // Longitude_oneMicrodegreeEast);
    const double latitude = mGeoPosition->latitude / vanetza::units::degree;
    basic.referencePosition.latitude = std::round(latitude * 1e6 * 10); // Latitude_oneMicrodegreeNorth);
    ...
    if (!mProtectedCommunicationZones.empty()) {
        ...
        for (const ProtectedCommunicationZone& zone : mProtectedCommunicationZones) {
            ...
            asn1->protectedZoneLatitude = std::round(zone.latitude_deg * 1e6 * 10); // Latitude_oneMicrodegreeNorth);
            asn1->protectedZoneLongitude = std::round(zone.longitude_deg * 1e6 * 10); // Longitude_oneMicrodegreeEast);
            if (zone.radius_m > 0) {
