...
SpeedValue_t buildSpeedValue(const vanetza::units::Velocity& v)
{
	...
	if (v >= upper) {
		...
		speed = round(v, centimeter_per_second) * 1; // SpeedValue_oneCentimeterPerSec;
	}
	return speed;
}
...
vanetza::asn1::Cam createCooperativeAwarenessMessage(const VehicleDataProvider& vdp, uint16_t genDeltaTime)
{
	...
	header.messageId = MessageId_cam; //ItsPduHeader__messageID_cam;
	...
	basic.stationType = TrafficParticipantType_passengerCar; // StationType_passengerCar;
	basic.referencePosition.altitude.altitudeValue = AltitudeValue_unavailable;
	basic.referencePosition.altitude.altitudeConfidence = AltitudeConfidence_unavailable;
	basic.referencePosition.longitude = round(vdp.longitude(), microdegree) * 10; // Longitude_oneMicrodegreeEast;
	basic.referencePosition.latitude = round(vdp.latitude(), microdegree) * 10; // Latitude_oneMicrodegreeNorth;
	...
	bvc.heading.headingConfidence = 10; // HeadingConfidence_equalOrWithinOneDegree;
	bvc.speed.speedValue = buildSpeedValue(vdp.speed());
	bvc.speed.speedConfidence = 1 * 3; // SpeedConfidence_equalOrWithinOneCentimeterPerSec * 3;
	...
	if (lonAccelValue >= -160.0 && lonAccelValue <= 161.0) {
		bvc.longitudinalAcceleration.longitudinalAccelerationValue = lonAccelValue * 1; //LongitudinalAccelerationValue_pointOneMeterPerSecSquaredForward;
	} else {
	...
	bvc.yawRate.yawRateValue = round(vdp.yaw_rate(), degree_per_second) * 1 * 100; // YawRateValue_degSec_000_01ToLeft * 100.0;
