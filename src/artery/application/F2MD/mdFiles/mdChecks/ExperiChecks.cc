...
#include <sys/types.h>
#include <sys/stat.h>
#include "../mdChecks/ExperiChecks.h"
#include <iostream>
#include <string>
#include <vector>
...
/**
   *  @brief create a new Experi checks configuration
*/
ExperiChecks::ExperiChecks(int version, unsigned long myPseudonym,
    ...
    LinkControl* LinkC, std::unordered_map<int, Coord>* realDynamicMap, inet::MACAddress myId, F2MD_CAMParameters* params)
{
...
double ExperiChecks::ProximityPlausibilityCheck(veins::Coord* testPosition, veins::Coord* myPosition, veins::Coord* myHeading)
{
    ...
    if (distance < params->MAX_PROXIMITY_RANGE_L) {
        if (distance < params->MAX_PROXIMITY_RANGE_W * 2 || (positionAngle < 90 && distance < (params->MAX_PROXIMITY_RANGE_W / cos((90 - positionAngle) * M_PI / 180)))) {
            ...
            for (std::pair<int, veins::Coord> x : *realDynamicMap) {
                if (x.first != myId.getInt()) {
...
/**
   *  @brief Run checks on the given BSM
   *  @param BSM* bsm to be analysed
   *  @param NodeTable* Node table containing all the node History for consistancy checks
   *  @result results of the checks performed on the given BSM
*/
BsmCheck ExperiChecks::CheckBSM(BSM* bsm,
    NodeTable* detectedNodes)
{
    ...
    if (!newNode) {
        double timeeee = mdmLib.calculateDeltaTime(bsm,senderNode->getLatestBSMAddr());
        ...
        EV_INFO << "bsm->getArrivalTime()" << bsm->getArrivalTime().dbl() << endl;
        EV_INFO << "senderNode->getLatestBSMAddr()->getArrivalTime().dbl())" <<bsm->getArrivalTime().dbl() << endl;
