...
void LinkControl::initialize(F2MD_CAMParameters * params,const traci::VehicleController* mVehicleController) {
    this->params = params;
    auto& lane_api = mVehicleController->getTraCI()->lane;

    for (auto const& id : lane_api.getIDList()) {
        
        auto slist = lane_api.getShape(id);
        std::vector<veins::Coord> slistcoord;

        // convert traciPos --> veins::Coord
        for (auto const shape : slist.value){
            veins::Coord coord = Coord(shape.x,shape.y,shape.z);
            slistcoord.push_back(coord);
        }

        std::vector<veins::Coord> vec1;
        vec1.insert(vec1.begin(), slistcoord.begin(),slistcoord.end());
        addShape(vec1);
    }
}
