...
public:
    ...
    void initVeReMiPrintable(std::string basePath, std::string serial, inet::MACAddress& realId, int parentId, int attackerType, const std::string curDate,double sliceTime, double sliceStartTime, double simTime);


    virtual const void traceJSON(std::string file, std::string JSONObject) const;
    void serializeGroundTruth(BSM *bsm);
    void serializeBeacon(BSM *bsm);
