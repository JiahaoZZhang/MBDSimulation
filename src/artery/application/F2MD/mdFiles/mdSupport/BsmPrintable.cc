...
void BsmPrintable::setBsm(BSM bsm) {
    this->bsm = bsm;
}

void BsmPrintable::setReceiverId(inet::MACAddress receiverId) {
...
std::string BsmPrintable::getBsmPrintableXml() {
    ...
    tempStr = tempStr + std::to_string(receiverId.getInt());
...
std::string BsmPrintable::getSelfBsmPrintHead(std::string myVType) {
    ...
    tempStr = jw.getSimpleTag("myId", std::to_string(receiverId.getInt()),
            true);
...
std::string BsmPrintable::getBsmPrintHead() {
    ...
    tempStr = jw.getSimpleTag("receiverId", std::to_string(receiverId.getInt()),
            true);
    jw.addTagToElement("Metadata", tempStr);
