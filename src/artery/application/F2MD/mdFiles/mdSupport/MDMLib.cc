...
double MDMLib::calculateDistanceXY(double xDistance, double yDistance){
    return sqrt(pow(xDistance, 2.0) + pow(yDistance, 2.0));
}

double MDMLib::calculateSpeedXY(double xSpeed, double ySpeed){
    return sqrt(pow(xSpeed, 2.0) + pow(ySpeed, 2.0));
}
...
double MDMLib::calculateDeltaTime(BSM * bsm1,
        BSM * bsm2) {
