...
void VeReMiPrintable::initVeReMiPrintable(std::string basePath, std::string serial, inet::MACAddress& realId, int parentId, int attackerType, const std::string curDate,double sliceTime, double sliceStartTime, double simTime) {
...
void VeReMiPrintable::serializeGroundTruth(BSM *bsm) {
    ...
    writer.Key("sender");
    writer.Uint(bsm->getSenderAddress().getInt());
...
void VeReMiPrintable::serializeBeacon(BSM *bsm) {
    veins::Coord *pos = &bsm->getSenderPos();
    veins::Coord *posc = &bsm->getSenderPosConfidence();
    veins::Coord *spd = &bsm->getSenderSpeed();
    veins::Coord *spdc = &bsm->getSenderSpeedConfidence();
