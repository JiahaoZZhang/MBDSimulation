    ...
    for (const PathPoint& point : concise_points) {
        ...
        while (!delta_time.is_negative() && path_points < scMaxPathPoints) {
            ...
            *(path_point->pathDeltaTime) = std::min(delta_time, scMaxDeltaTime).total_milliseconds() /
                10 * 1 ;//PathDeltaTime::PathDeltaTime_tenMilliSecondsInPast;
            path_point->pathPosition.deltaLatitude = (delta_latitude / scMicrodegree).value() * 10 ;
                // DeltaLatitude::DeltaLatitude_oneMicrodegreeNorth;
            path_point->pathPosition.deltaLongitude = (delta_longitude / scMicrodegree).value() * 10 ;
...
units::Length distance(const ReferencePositionWithConfidence_t& a, const ReferencePositionWithConfidence_t& b)
{
    ...
    if (is_available(a) && is_available(b)) {
        GeodeticPosition geo_a {
            GeoAngle { a.latitude / 10 * microdegree }, // Latitude_oneMicrodegreeNorth
            GeoAngle { a.longitude / 10 * microdegree } // Longitude_oneMicrodegreeEast
        };
        GeodeticPosition geo_b {
            GeoAngle { b.latitude / 10 * microdegree }, // Latitude_oneMicrodegreeNorth
            GeoAngle { b.longitude / 10 * microdegree } // Longitude_oneMicrodegreeEast
        };
...
bool is_available(const ReferencePositionWithConfidence& pos)
{
...
void copy(const PositionFix& position, ReferencePosition& reference_position) {
    reference_position.longitude = round(position.longitude, microdegree) * 10; //Longitude_oneMicrodegreeEast;
    reference_position.latitude = round(position.latitude, microdegree) * 10; //Latitude_oneMicrodegreeNorth;
...
AltitudeValue_t to_altitude_value(units::Length alt)
{
    ...
    if (!isnan(alt)) {
        ...
        return 100.0 * (alt / units::si::meter); // AltitudeValue_oneCentimeter * 100.0 * (alt / units::si::meter);
    } else {
...
void print_indented(std::ostream& os, const asn1::Cam& message, const std::string& indent, unsigned level)
{
    ...
    prefix("Station ID") << header.messageId << "\n";
    --level;
